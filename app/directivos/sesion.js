...
export const NOMBRE_COOKIE_SESION = "seg_sesion_directivos";

const DURACION_MS = 8 * 60 * 60 * 1000; // 8 horas
export const DURACION_SESION_SEGUNDOS = DURACION_MS / 1000;

function obtenerSecreto() {
  const secreto = process.env.CLAVE_DIRECTIVOS_SECRETO;
  if (!secreto) {
    throw new Error("Falta configurar la variable de entorno CLAVE_DIRECTIVOS_SECRETO");
  }
  return secreto;
}
...
export function claveCorrecta(claveIngresada) {
  const claveReal = process.env.CLAVE_DIRECTIVOS;
