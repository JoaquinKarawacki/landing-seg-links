...
export const NOMBRE_COOKIE_SESION = "seg_sesion_admin_documentos";

const DURACION_MS = 8 * 60 * 60 * 1000; // 8 horas
export const DURACION_SESION_SEGUNDOS = DURACION_MS / 1000;

function obtenerSecreto() {
  const secreto = process.env.CLAVE_ADMIN_DOCUMENTOS_SECRETO;
  if (!secreto) {
    throw new Error(
      "Falta configurar la variable de entorno CLAVE_ADMIN_DOCUMENTOS_SECRETO"
    );
  }
  return secreto;
}
...
export function claveCorrecta(claveIngresada) {
  const claveReal = process.env.CLAVE_ADMIN_DOCUMENTOS;
