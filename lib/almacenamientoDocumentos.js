...
// autoClose: sin esto el file descriptor queda abierto después de mandar el
// archivo, hasta que lo cierre el garbage collector.
export async function abrirFlujoLectura(nombreArchivo) {
  const handle = await open(
    path.join(/* turbopackIgnore: true */ rutaBase(), nombreArchivo)
  );
  return handle.readableWebStream({ autoClose: true });
}
